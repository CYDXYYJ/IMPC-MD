...
def execute_trajectory(item):
    
    swarm,agent_list,interval=item

    interval_time=agent_list[0].h*interval
    ...
    while True:
        ...
        runtime=time.time()-start
        if runtime > interval_time-0.01:
            break
        time.sleep(0.02)
    
    return None

def land(swarm,targetHeight, duration):
    ...
    for i in range(10):
        ...
        for cf in swarm.allcfs.crazyflies:
            ...
            cf.cmdFullState(pos, vel, acc, yaw, omega)
            j+=1
        swarm.timeHelper.sleep(duration/10)
